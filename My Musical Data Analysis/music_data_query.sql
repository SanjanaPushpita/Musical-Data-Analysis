...
SELECT billing_city, SUM(total) AS i_total
FROM invoice
...
SELECT c.first_name, c.last_name, c.email AS mail
FROM customer AS c
JOIN invoice AS i
ON c.customer_id = i.customer_id
...
SELECT artist.name AS Name, COUNT(artist.artist_id) AS Number_of_Songs 
FROM artist
